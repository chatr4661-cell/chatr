...
serve(async (req) => {
  ...
  try {
    ...
    if (GOOGLE_API_KEY && GOOGLE_CX_ID) {
      ...
      console.log(`Calling Google Custom Search: "${searchQuery}" (gl=${effectiveCountry?.toLowerCase() || 'in'})`);
      ...
      try {
        ...
        if (googleResponse.ok && googleData.items) {
          ...
          console.error(`Google API error - Status: ${googleResponse.status}, Code: ${errorCode}, Reason: ${errorReason}`);
    ...
    try {
      ...
      if (!aiError && aiData) {
        aiAnswer = {
          ...
          images: aiData.images || imageResults
        };
      } else {
...
// DuckDuckGo fallback search - FIXED HTML parsing
async function searchDuckDuckGo(
  ...
  // Approach 1: DuckDuckGo HTML Scraping
  try {
  ...
  // Approach 3: Wikipedia search as final fallback
  if (results.length < 3) {
    try {
...
function classifyResult(query: string, item: any): string {
  ...
  if (combined.includes('near me') || combined.includes('plumber') || combined.includes('electrician') || combined.includes('salon') || combined.includes('mechanic') || combined.includes('repair')) {
    return 'local_business';
  }

  if (url.includes('zomato') || url.includes('swiggy') || combined.includes('restaurant') || combined.includes('food') || combined.includes('cafe')) {
    return 'restaurant';
  }

  if (combined.includes('doctor') || combined.includes('hospital') || combined.includes('clinic') || combined.includes('pharmacy') || combined.includes('medical')) {
    return 'health';
  }

  if (url.includes('booking.com') || url.includes('tripadvisor') || url.includes('makemytrip') || combined.includes('hotel') || combined.includes('flight') || combined.includes('travel')) {
    return 'travel';
  }

  if (url.includes('amazon') || url.includes('flipkart') || url.includes('myntra') || combined.includes('buy') || combined.includes('shop') || combined.includes('price')) {
    return 'ecommerce';
  }

  if (url.includes('naukri') || url.includes('indeed') || url.includes('linkedin') || combined.includes('job') || combined.includes('career') || combined.includes('hiring')) {
    return 'job';
  }

  if (url.includes('coursera') || url.includes('udemy') || combined.includes('course') || combined.includes('learn') || combined.includes('education')) {
    return 'education';
  }
